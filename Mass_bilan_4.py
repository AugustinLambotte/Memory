...
if __name__ == '__main__':
    ...
    for year in range(year_,year_end):
        for month in range(1,13):
            ...
            for day in range(len(recorded_si_drift)):
                ...
                transport = np.where(abs(recorded_si_drift[day]) >0 , recorded_si_drift[day] * daily_interp_sit[day], np.nan)
        ...
        plt.xlabel('month')
        plt.ylabel('[km^3]')
        plt.savefig(f"Plots/Fram_strait/4.0/{year}_Mass_bilan.png")
    ...
    plt.xlabel('year')
    plt.ylabel('[km^3]')
    plt.savefig(f"Plots/Fram_strait/4.0/Annual_mean_mass_bilan.png")
